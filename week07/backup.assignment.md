...
The stub file [`double_linked_list_assignment.py`](./double_linked_list_assignment.py) contains a `DoubleLinkedList` class. `__init__`, `add`, `get_count`, `close_loop`, and `has_loop` are already complete. Your job is to implement six methods: one that finds the middle node, two that inspect whether a list's links go only one way, and three that detect broken or looping pointers.
...
**For comparison: the slow/fast cursor approach.** You do not implement this -- it is shown here only so you can see the alternative from the June 30 session and understand why `get_middle_by_count` is the better choice when a count is already available:

```python
# Illustration only -- not a method you need to write.
def get_middle_two_cursor_example(self):
  middle_node = None
...
Both approaches land on the same node. The difference is what each one is allowed to assume. The cursor race never reads `self._count` -- it has to discover where the middle is by racing a fast cursor to the end of the list, which costs a full **O(n)** traversal no matter how the race is run. `get_middle_by_count` gets to skip that discovery step entirely: because `add` already keeps `self._count` current, finding the middle *index* is a single lookup and integer division, **O(1)**, and only the direct walk to that index remains. The lesson is the same one from `_count` throughout this assignment: tracking a value as you go is cheaper than rediscovering it every time someone asks.

---

## Part 2: `which_direction`
...
After implementing all six methods, run [`double_linked_list_assignment.py`](./double_linked_list_assignment.py) from the `week07` directory:
...
The first three lines test `get_middle_by_count` on a 5-station list, a 4-station list, and an empty list. The next six test `which_direction` and `is_unidirectional` on a normal list plus the forward-only and backward-only fixtures built by `_build_one_directional`. The next two test `has_discontinuity` on a normal list and one with a deliberately broken `_prev`. The next two test `has_infinite_loop` on a normal list and a fully wrapped one. The last four test `has_loop` and `has_loop_instant` on a normal list and a looped one.

Edge cases to verify manually:

- All six methods return their "empty" value (`None`, `0`, `False`, as appropriate) on a freshly constructed `DoubleLinkedList()` with nothing added.
...
## Part 7: Reflection -- Pointer Maintenance, Then and Now
